...
def create_file_structure_and_copy_template(review_name_input, release_year, date_offset):
    ...
    review_publish_year = tomorrow_at_1030.strftime("%Y")
    review_publish_date = tomorrow_at_1030.strftime("%Y-%m-%d")
    full_review_publish_date = tomorrow_at_1030.strftime("%Y-%m-%d %H:%M:%S") + " " + gmt_offset
...
if __name__ == "__main__":
    date_offset_input = input("Offset (defaults to 1): ").strip()
    date_offset = date_offset_input if date_offset_input else 1
    ...
    create_file_structure_and_copy_template(review_name_input, release_year, date_offset)